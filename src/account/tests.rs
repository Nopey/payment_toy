...
fn process_tx_skips_dup_deposits() {
    ...
    for _ in 0..10 {
        assert_eq!(
            Err(Error::DuplicateTransaction(tx_id)),
...
fn process_tx_denies_deposit_in_locked_account() {
    ...
    let withdrawal = Transaction::new(Action::new_deposit(deposit_amount), client, deposit_id);
    assert_eq!(
        Err(Error::AccountLockedFundsFrozen(deposit_id)),
...
fn process_tx_denies_withdrawal_in_locked_account() {
    ...
    assert_eq!(
        Err(Error::AccountLockedFundsFrozen(withdrawal_id)),
        account.process_transaction(&withdrawal, &mut tx_history)
    );
...
fn process_tx_allows_dispute_and_chargeback_in_locked_account() {
    ...
    assert_eq!(
        Ok(()),
        account.process_transaction(&chargeback, &mut tx_history)
    );
...
fn process_tx_allows_dispute_and_resolve_in_locked_account() {
    ...
    assert_eq!(
        Ok(()),
        account.process_transaction(&resolve, &mut tx_history)
    );
...
fn process_tx_skips_dup_withdrawals() {
    ...
    for _ in 0..10 {
        assert_eq!(
            Err(Error::DuplicateTransaction(withdrawal_id)),
            account.process_transaction(&withdrawal, &mut tx_history)
        );
    }
    assert_eq!(
...
fn duplicate_disputes_are_rejected() {
    ...
    for _ in 0..10 {
        ...
        assert_eq!(
            Err(Error::DuplicateDispute(deposit_id)),
            account.process_transaction(&dispute, &mut tx_history)
        );
...
fn chargebacks_dont_free_txid() {
    ...
    assert_eq!(
        Err(Error::DuplicateTransaction(deposit_id)),
        second_account.process_transaction(&second_deposit, &mut tx_history)
    );
}
